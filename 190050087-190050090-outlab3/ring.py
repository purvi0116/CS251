...
class RingInt:
	def __init__(self, characteristic, value):
		...
		self.value=int(value)%self.char
	...
	def __sub__(self, other):
		if self.char == other.char:
			obj  = RingInt(self.char, abs(self.value - other.value)%self.char)
			return(obj)
		else:
			raise ValueError('UNDEFINED')
	...
	def __truediv__(self, other):
		if self.char == other.char and other.value%self.char !=0 :
			...
			for i in range(self.char):
				if (other.value*i)%self.char == self.value:
	...
	def __pow__(self, exp):
		obj  = RingInt(self.char, pow(self.value, exp)%self.char)
		return(obj)
